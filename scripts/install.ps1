#
# vf CLI Installation Script for Windows
...
#   $env:VF_VERSION     - Specific version to install (default: latest)
#
...
$Repo = "voiceflow/cli"
$BinaryName = "vf.exe"
$DefaultInstallDir = Join-Path $env:LOCALAPPDATA "Programs\vf"
$InstallDir = if ($env:VF_INSTALL_DIR) { $env:VF_INSTALL_DIR } else { $DefaultInstallDir }
$Version = if ($env:VF_VERSION) { $env:VF_VERSION } else { "latest" }
...
function Install-CLI {
    Write-ColorOutput "Installing vf CLI..." -Color Green
    ...
    $archiveName = "vf_Windows_$arch.zip"
    $downloadUrl = "https://github.com/$Repo/releases/download/$Version/$archiveName"

    Write-ColorOutput "Downloading from: $downloadUrl" -Color Cyan

    # Create temporary directory
    $tempDir = Join-Path $env:TEMP "vf-install-$(New-Guid)"
    ...
    try {
        ...
        Write-ColorOutput "vf $Version has been installed to $binaryPath" -Color Green
        ...
        Write-ColorOutput "Installation successful! Run 'vf --help' to get started." -Color Green
